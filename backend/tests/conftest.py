...
def as_role():
    ...
    def _apply(
        ...
        *,
        ...
        # Chat is public now, so its quota guard resolves the caller through
        # `get_optional_user` rather than `get_current_user`. Overriding only
        # the latter would leave the real dependency in place, and the real
        # one opens a database session -- so the test would fail reaching for
        # Postgres rather than exercising the route.
