...
class TestTheQuotaReplacesTheGate:
    async def test_an_anonymous_caller_is_limited_by_address(self, client, monkeypatch):
        ...
        # `Limit` is frozen, so the whole object is replaced. A distinct
        # scope name keeps each test's bucket to itself.
        monkeypatch.setattr(
    ...
    async def test_the_global_ceiling_stops_the_deployment_overspending(
        ...
        reads as untouched. Counting every turn into one bucket is what makes
        the endpoint affordable to leave open.
        """
        monkeypatch.delenv(rate_limit.DISABLE_ENV, raising=False)
        monkeypatch.setattr(
        ...
        # anyone can send it -- and every request would share one address.
        # The test would then pass on the per-caller limit and prove nothing
        # about rotation, which is the whole scenario.
        ...
        assert codes[:2] == [c for c in codes[:2] if c != 429], "ceiling fired too early"
...
class TestSignupIsGone:
    def test_the_signup_endpoint_no_longer_exists(self):
        ...
        front door makes that question stop existing. Accounts are for staff
        again, created by an admin.
        ...
        failing to resolve a Docker hostname. The router carries the routes;
        the app only mounts it.
        """
