...
class EntityRef:
    ...
    #: 479 entities are connected but hold no facts at all, so without it
    #: every node on the canvas looks equally promising and half of them are
    #: dead ends. Zero when the caller did not ask for counts.
    fact_count: int = 0
