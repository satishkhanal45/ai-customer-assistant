...
* ``visitor`` — anyone. Created by self-service signup from the login page,
  with no vetting: a prospective client asking what you charge, someone
  checking what services exist. They can hold a conversation and nothing
  else. Everything a visitor can reach is read-only by construction, which
  is what makes an open signup endpoint safe to expose.
...
different populations once signup is open to the public: a stranger and a
colleague should not share a permission set merely because both are "signed
in". The alternative considered was renaming ``member`` to ``visitor`` and
keeping two roles, which would have been strictly worse -- the role travels
inside the JWT, so renaming it invalidates every live session, and it needs a
data migration and a rewrite of every call site. Adding a tier *below* the
existing one costs neither.

The ordering is what makes that cheap. ``satisfies`` is a rank comparison, so
inserting ``visitor`` below ``member`` made every existing
...
#: Self-service signup does not use this -- it names `VISITOR` explicitly, so
#: that changing this default can never widen what a stranger is granted.
