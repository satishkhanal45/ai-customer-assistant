...
(function (NS) {
  ...
  /* A visitor is a stranger -- a prospective client asking what you charge,
     not a colleague. They have one page, so the conversation *is* the product.
  ...
  function initVisitor(container) {
    ...
    head.innerHTML =
      ...
      '<strong>Alpinist Studios</strong>' +
    ...
    if (!t || !t.messages.length) {
      area.appendChild(NS.utils.el('div', { class: 'welcome' },
        ...
        /* A visitor arrives without knowing what this thing knows, so the
           openers double as a statement of scope. */
  ...
  /* Source names are internal artefacts: upload filenames and crawled URLs.
     ...
     typo intact -- as the authority behind an answer. This hides nothing (the
     same documents produced the answer either way); it stops the citation
     reading like a leaked directory listing.

     Anything unrecognisable falls through unchanged rather than being
     replaced by something vague, because a wrong-but-specific citation is
     easier to report than a confident "our documentation". */
      ...
      if (m.citations && m.citations.length) {
        ...
        var cites = m.citations.map(function (c) {
          ...
          /* Version numbers are corpus bookkeeping. They tell a curator which
             ingestion produced a passage and a prospective client nothing. */
  ...
  function renderAssistant(content) {
    ...
    var html = lines.map(function (line) {
      ...
      /* Every branch goes through `inlineMarkup`, which escapes.
         ...
         model's output derives from ingested documents, so that was a path
         from "someone uploads a document" to "script runs in another user's
         browser" -- and it matters more now the reader may be a visitor
